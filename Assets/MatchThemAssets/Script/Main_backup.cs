...
//Change the class name to "Main" before using
public class Main_Backup : MonoBehaviour
{
    ...
    public bool active;
    private object[] previousTweenInfos;
    ...
    void Update()
    {
      if (active){
        ...
        if (HOTween.GetTweenInfos() == null)    
        {
            var MatchesX = FindMatchX(_arrayOfShapes);
            var MatchesY = FindMatchY(_arrayOfShapes);
            //If we find a matched tiles on the X or Y Axis
            var isBombX = MatchesX.Pop();
            var isBombY = MatchesY.Pop();
            if (!(isBombX is bool) || !(isBombY is bool))
            {
                throw new InvalidOperationException("Last object in match stack must be a bool");
            }
            if (MatchesX.Count > 0 || MatchesY.Count > 0)
            {
                //Update the score
                _scoreTotal += MatchesX.Count * _scoreIncrement;
                // int e = Matches.get(Matches.size() - 1);
                // Debug.Log(e);
                //TODO Explode this bomb

                foreach (GameObject go in MatchesX)
                {
    ...
    // Find Match-3 Tile
    private Stack FindMatchX(GameObject[,] cells)
    {
        ...
        for (var x = 0; x <= cells.GetUpperBound(0); x++)
        {
            for (var y = 0; y <= cells.GetUpperBound(1); y++)
            {
                ...
                if (thiscell.name == "Empty(Clone)" || gem.pill) continue;
                int matchCount = 0;
                ...
                for (y1 = y + 1; y1 <= y2; y1++)
                {
                    var gemxy1 = cells[x, y1].GetComponent<Gem>();
                    if (cells[x, y1].name == "Empty(Clone)" || thiscell.name != cells[x, y1].name || gemxy1.pill) break;
                    matchCount++;
                }
                ...
                if (matchCount >= 2)
                {
                    ...
                    for (var y3 = y; y3 <= y1; y3++)
                    {
                        ...
                        {
                            if ((gem.bomb || gem.pill) && cells[x, y3] == thiscell) continue;
                            stack.Push(cells[x, y3]);
                        }
                    }
                }
            }
    ...
    private Stack FindMatchY(GameObject[,] cells)
    {
        //creating an arraylist to store the matching tiles
        Stack stack = new Stack();
        var isBomb = false;
        ...
        for (var y = 0; y < cells.GetUpperBound(1) + 1; y++)
        {
            for (var x = 0; x < cells.GetUpperBound(0) + 1; x++)
            {
                ...
                for (x1 = x + 1; x1 <= x2; x1++)
                {
                    ...
                    if (cells[x1, y].name == "Empty(Clone)" || thiscell.name != cells[x1, y].name || gemx1y.pill) break;
                    matchCount++;
                }
                if (matchCount >= 2)
                {
                    ...
                    for (var x3 = x; x3 <= x1; x3++)
                    {
                        ...
                        {
                            if ((gem.bomb || gem.pill) && cells[x3, y] == thiscell) continue;
                            stack.Push(cells[x3, y]);
                        }
                    }
                }
            }
